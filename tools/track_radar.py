...
def main() -> None:
    ...
    shutil.copyfile(os.path.join(args.workspace, 'tools/track_radar.sh'), os.path.join(root_path, 'track.sh'), follow_symlinks=True)
    ...
    from tracker_radar import PubTracker, RADAR_FUSION_NAMES
    tracker = PubTracker(
        hungarian=False,
    ...
    # load radar trackers
    if args.radar_fusion:
        with open(args.radar_tracker_path, 'rb') as f:
            radar_trackers = json.load(f)['key_results']
    ...
    for i in tqdm(range(len_frames)):
        ...
        if args.radar_tracker_path is not None:
            radar_tracker = deepcopy(radar_trackers[token])
        else:
            radar_tracker = []
        outputs = tracker.step_centertrack(det, time_lag, radar_tracker)
